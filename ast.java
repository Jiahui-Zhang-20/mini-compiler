...
abstract class ASTnode {
    ...
    // hash table of (string, label) pairs
    // so each distinct string is declared once
    static HashMap<String, String> stringTable;
...
class FnBodyNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab) {
        // propagate function name to statement list node
        myStmtList.setFnName(this.myFnName);
    ...
    public void codeGen() {
        ...
        // stack pointer should already be moved
        // for any local variable declarations

        // run code generation on all statements
        myStmtList.codeGen();
    }

    public void setFnName(String fnName) {
        this.myFnName = fnName;
    }

    private String myFnName;
    // 2 kids
    private DeclListNode myDeclList;
...
class StmtListNode extends ASTnode {
    ...
    public void codeGen() {
        ...
        // call code generation on each statement
        Iterator<StmtNode> it = this.myStmts.iterator();
        while (it.hasNext()) {
            it.next().codeGen();
        }
    }
...
class ExpListNode extends ASTnode {
    ...
    public void codeGen() {
        Iterator<ExpNode> it = this.myExps.iterator();
        while (it.hasNext()) { // if there is at least one element
            // evaluate expression and push parameter onto stack
            it.next().codeGen();
        }
    }
...
abstract class DeclNode extends ASTnode {
    ...
    // code generation for declaration nodes
    // VarDeclNode: variable declarations only generate code if global
    // fnDeclNode: function declarations need to generate code
    // formalDeclNode: formal declarations do not need to generate code
    // structDeclNode: struct declarations not within scope of assignment
...
class VarDeclNode extends DeclNode {
            ...
            try {
                if (myType instanceof StructNode) {
                    ...
                    if (!globalTab.isGlobalScope()) {
                        ...
                        globalTab.setOffset(offset - 4); // everything is int or
                                                         // bool
                    } else {
                        sym.setOffset(1);
                    }
    ...
    public void codeGen() {
        ...
        if (this.myId.sym().isGlobal()) {
            ...
            // indicate global variable follows
            Codegen.generate(".data");
            // set alignment to 4
            Codegen.generate(".align 2");
            // declare global variable
            Codegen.generateLabeled("_" + this.myId.name(), ".space 4", "");

            // local variable declaration
        }

        // local variable declarations do not generate code
        // it is taken care of when we move the stack pointer
        // accordingly during function calls
...
class FnDeclNode extends DeclNode {
    ...
    public TSym nameAnalysis(SymTable symTab) {
        ...
        // propagate function name to function body
        this.myBody.setFnName(this.myId.name());
    ...
    public void codeGen() {
        ...
        // special label for main entry
        if (this.myId.isMain()) {
            Codegen.genLabel("main", "METHOD ENTRY");
            Codegen.genLabel("__start", "add __start label for main only");
            // label for function entry
        } else {
            Codegen.genLabel("_" + this.myId.name());
        }

        // push return address
        Codegen.genPush(Codegen.RA);
        // push control link (caller's frame pointer)
        Codegen.genPush(Codegen.FP);
        // set callee frame pointer (this function)
        // at first parameter
        Codegen.generateWithComment("addu",
                "set frame pointer to first parameter", Codegen.FP, Codegen.SP,
                String.valueOf(((IdNode) this.myId).paramSize() + 8));
        // the stack pointer moved for local variables
        Codegen.generate("subu", Codegen.SP, Codegen.SP, this.myId.localSize());
        ...
        ///// epilogue /////
        Codegen.genLabel("_" + this.myId.name() + "_Exit");
        // load caller's return address into $ra
        Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP,
                -((IdNode) this.myId).paramSize(),
                "load caller's return address");
        // move frame pointer to $t0 (which will be caller's stack pointer)
        Codegen.generateWithComment("move", "move frame pointer to $t0",
                Codegen.T0, Codegen.FP);
        // set frame pointer to caller's frame pointer
        Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP,
                -((IdNode) this.myId).paramSize() - 4,
                "set frame pointer back to caller's frame pointer");
        // set the stack pointer to $t0 which holds the callee's frame pointer
        Codegen.generateWithComment("move",
                "set stack pointer to callee's frame pointer", Codegen.SP,
                Codegen.T0);

        // main function exits with syscall
        if (this.myId.isMain()) {
            Codegen.generateWithComment("li", "set syscall code to exit",
                    Codegen.V0, String.valueOf(10));
            Codegen.generateWithComment("syscall", "exit main");
            // function returns with jump register instruction
        } else {
            Codegen.generateWithComment("jr", "return control to the caller",
                    Codegen.RA);
        }
...
class FormalDeclNode extends DeclNode {
    ...
    public void codeGen() {
        // formal declarations are similar to local variables
        // in that they don't need to generate code
    }
...
abstract class StmtNode extends ASTnode {
    ...
    // code generation must be implemented
    // for every statement node
    abstract public void codeGen();
...
class AssignStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        // code generation of assign expression
        this.myAssign.codeGen();

        // pop the value off the stack
        // because it is not needed anymore
        // $t0 will just contain garbage value
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        if (this.myExp instanceof IdNode) {
            ...
            ((IdNode) this.myExp).codeGen();
            // pushes the address onto the stack
            ((IdNode) this.myExp).genAddress();
            // pop address
            Codegen.genPop(Codegen.T1);
            // pop value
            Codegen.genPop(Codegen.T0);
            // increment value in $t0
            Codegen.generateWithComment("add",
                    "increment " + ((IdNode) this.myExp).name(), Codegen.T0,
                    Codegen.T0, String.valueOf(1));
            // assign incremented value to address
            Codegen.generateIndexed("sw", Codegen.T0, Codegen.T1, 0,
                    "store incremented value to "
                            + ((IdNode) this.myExp).name());
        } else {
            // cannot happen without struct dot access
        }
...
class PostDecStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        if (this.myExp instanceof IdNode) {
            ...
            // pop value
            Codegen.genPop(Codegen.T0);
            // decrement value in $t0
            Codegen.generateWithComment("sub",
                    "decrement " + ((IdNode) this.myExp).name(), Codegen.T0,
                    Codegen.T0, String.valueOf(1));
            // assign decremented value to address
            Codegen.generateIndexed("sw", Codegen.T0, Codegen.T1, 0,
                    "store decremented value to "
                            + ((IdNode) this.myExp).name());
        } else {
            // cannot happen without struct dot access
        }
...
class ReadStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        // put read code into $v0
        Codegen.generateWithComment("li", "load code for read", Codegen.V0,
                String.valueOf(5));
        // make system call to read into $v0
        Codegen.generateWithComment("syscall", "read");
        // push read value onto stack
        Codegen.genPush(Codegen.V0);

        if (this.myExp instanceof IdNode) {
            // push id address onto stack
            ((IdNode) this.myExp).genAddress();

        } else {
            // this cannot occur with struct dot access
        }

        // load the id address into $t1
        Codegen.genPop(Codegen.T1);
        // load the read value into $t0
        Codegen.genPop(Codegen.T0);

        // store the read value in the id address
        Codegen.generateIndexed("sw", Codegen.T0, Codegen.T1, 0,
                "read value to " + ((IdNode) this.myExp).name());
    }
...
class WriteStmtNode extends StmtNode {
    ...
    int writeSysCallCode = 0;

    public void codeGen() {

        // pushes value onto stack
        myExp.codeGen();
        // pops value off stack into $a0
        Codegen.genPop(Codegen.A0);

        // write integer
        if (myType.isIntType()) {
            // set code for writing integer
            writeSysCallCode = 1;
            // write boolean
        ...
        // set code in $v0
        Codegen.generateWithComment("li", "load code for write", Codegen.V0,
                String.valueOf(writeSysCallCode));
        // make system call to write
        Codegen.generateWithComment("syscall", "write " + this.myType);
...
class IfStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab) {
        ...
        symTab.addScope();
        myDeclList.nameAnalysis(symTab);
        this.myStmtList.setFnName(myFnName);
        myStmtList.nameAnalysis(symTab);
        try {
            symTab.removeScope();
        } catch (EmptySymTableException ex) {
    ...
    public void codeGen() {
        ...
        // create new done label
        String doneLabel = ".D" + Codegen.nextLabel().substring(2);

        // push expression result onto stack
        this.myExp.codeGen();
        // pop expression result into $t0
        Codegen.genPop(Codegen.T0);
        // jump to done if false
        Codegen.generateWithComment("beq", "skip if-then body", Codegen.T0,
                Codegen.FALSE, doneLabel);
        // fall through to if-body
        this.myStmtList.codeGen();
        // generate done label
...
class IfElseStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab) {
        ...
        myThenStmtList.setFnName(myFnName);
        ...
        myElseStmtList.setFnName(myFnName);
    ...
    public void codeGen() {
        ...
        // create new done label
        String doneLabel = ".D" + Codegen.nextLabel().substring(2);
        String elseLabel = ".E" + Codegen.nextLabel().substring(2);
        ...
        Codegen.generateWithComment("beq", "jump to else body", Codegen.T0,
                Codegen.FALSE, elseLabel);
        // fall through to then-body
        this.myThenStmtList.codeGen();
        // jump to done label
        Codegen.generateWithComment("b", "leave if-then-else statement",
                doneLabel);

        Codegen.genLabel(elseLabel);
        // process else-body
        this.myElseStmtList.codeGen();
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        // generate while expression label
        String whileLabel = ".W" + Codegen.nextLabel();
        // generate done label
        String doneLabel = ".D" + Codegen.nextLabel();

        // generate while-condition label
        Codegen.genLabel(whileLabel);
        // push expression result onto stack
        this.myExp.codeGen();
        // load expression result into $t0
        Codegen.genPop(Codegen.T0);
        // jump to done if false
        Codegen.generateWithComment("beq", "leave while loop", Codegen.T0,
                Codegen.FALSE, doneLabel);
        // fall through to while-body
        this.myStmtList.codeGen();
        // jump back to while-expression
        Codegen.generateWithComment("b", "jump back to while-condition",
                whileLabel);
...
class ReturnStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        if (this.myExp != null) {
            // push return expression onto the stack
            this.myExp.codeGen();
        } else {
            // push garbage value from T0
            Codegen.genPush(Codegen.T0);
        }

        // pop returned value to %v0
        Codegen.genPop(Codegen.V0);

        // return from function
        Codegen.generateWithComment("b",
                "jump to function exit for " + this.myFnName,
                "_" + this.myFnName + "_Exit");
    }
...
abstract class ExpNode extends ASTnode {
    ...
    // code generation must be implemented
    // for every expression node so the result
    // value can be pushed onto the stack
    abstract public void codeGen();
...
class IntLitNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        // load value into $t0
        Codegen.generateWithComment("li",
                "load value " + String.valueOf(this.myIntVal), Codegen.T0,
                String.valueOf(this.myIntVal));
        Codegen.genPush(Codegen.T0);
...
class StringLitNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        // this string has already been declared globally
        if (ASTnode.stringTable.containsKey(this.myStrVal)) {
            strLabel = ASTnode.stringTable.get(this.myStrVal);
            // declare string globally
        } else {
            // generate string label
            Codegen.generate(".data");
            strLabel = ".S" + Codegen.nextLabel().substring(2);
            Codegen.generateLabeled(strLabel, ".asciiz " + this.myStrVal, "");
        }

        // push string label onto stack
        Codegen.generate(".text");
        Codegen.generateWithComment("la", "load string " + strLabel, Codegen.T0,
                strLabel);
        Codegen.genPush(Codegen.T0);
        // add string and label to hash table
        ASTnode.stringTable.put(this.myStrVal, strLabel);
    }
...
class TrueNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        // load true into $t0
        Codegen.generateWithComment("li", "load true", Codegen.T0,
                Codegen.TRUE);
        Codegen.genPush(Codegen.T0);
    }
...
class FalseNode extends ExpNode {
    ...
    public void codeGen() {
        // load true into $t0
        Codegen.generateWithComment("li", "load false", Codegen.T0,
                Codegen.FALSE);
        Codegen.genPush(Codegen.T0);
    }
...
class IdNode extends ExpNode {
    ...
    /**
     * Return the total number of bytes for all locals. HINT: This method may be
     * useful during code generation.
     */
    public int localSize() {
        if (!(mySym instanceof FnSym)) {
            throw new IllegalStateException(
                    "cannot call local size on a non-function");
        }
        return ((FnSym) mySym).getLocalSize();
    }

    /**
     * Return the total number of bytes for all params. HINT: This method may be
     * useful during code generation.
     */
    ...
    /**
     * Is this function main? HINT: This may be useful during code generation.
     */
    ...
    // generate a jump and link instruction
    public void genJumpAndLink() {
        ...
        // jump and link to called function
        Codegen.generateWithComment("jal", "jump and link to " + this.name(),
                fnLabel);

    }

    // copy the value of the global/local variable into a register
    // then push the value onto the stack
    public void codeGen() {

        // the id is global
        if (this.sym().isGlobal()) {
            Codegen.generateWithComment("lw", "load global value", Codegen.T0,
                    "_" + this.name());
            // the id is local
        } else {
            Codegen.generateIndexed("lw", Codegen.T0, Codegen.FP,
                    this.mySym.getOffset(), "load local value");
        }
    ...
    // load the address of the id into a register
    // then push it onto the stack
    public void genAddress() {

        // the id is global
        if (this.sym().isGlobal()) {
            Codegen.generateWithComment("la", "load global address", Codegen.T0,
                    "_" + this.name());
            // the id is local
        } else {
            Codegen.generateIndexed("la", Codegen.T0, Codegen.FP,
                    this.sym().getOffset(), "load local address");
        }
...
class AssignNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        // and pushed onto stack
        this.myExp.codeGen();

        // the lhs is evaluated
        // and pushed onto stack
        if (this.myLhs instanceof IdNode) {
            ((IdNode) this.myLhs).genAddress();
        } else {
            // this should not happen if we don't
            // consider struct dot access
        }
        ...
        // do assignment of rhs value
        // to lhs address
        Codegen.generateIndexed("sw", Codegen.T0, Codegen.T1, 0,
                "assign to " + ((IdNode) this.myLhs).name());

        // push the rhs value onto the stack
        // because assignNode is an ExpNode
        // (e.g. a = b = c)
        Codegen.genPush(Codegen.T0);
    }
...
class CallExpNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        // jump to the function and store the program
        // counter in $ra
        this.myId.genJumpAndLink();
        // push the returned value onto the stack
        // we do this even if the return is void
        Codegen.genPush(Codegen.V0);
...
class UnaryMinusNode extends UnaryExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop(Codegen.T0);
        // negate value
        Codegen.generateWithComment("sub", "negate value", Codegen.T0, "$zero",
                Codegen.T0);
        // push negated value back onto the stack
        Codegen.genPush(Codegen.T0);
...
class NotNode extends UnaryExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop(Codegen.T0);
        // negate value
        Codegen.generateWithComment("seq", "negate boolean", Codegen.T0,
                Codegen.T0, Codegen.FALSE);
        // push negated boolean back onto the stack
        Codegen.genPush(Codegen.T0);
...
class PlusNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.genPop(Codegen.T0);
        // load sum into $t0
        Codegen.generateWithComment("add", "addition", Codegen.T0, Codegen.T0,
                Codegen.T1);
        // push sum onto stack
        Codegen.genPush(Codegen.T0);
...
class MinusNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
        ...
        // pop lhs value into %t0
        Codegen.genPop(Codegen.T0);
        // load difference into $t0
        Codegen.generateWithComment("sub", "subtraction", Codegen.T0,
                Codegen.T0, Codegen.T1);
...
class TimesNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
        ...
        // pop lhs value into %t0
        Codegen.genPop(Codegen.T0);
        // load product into $t0
        Codegen.generateWithComment("mul", "product", Codegen.T0, Codegen.T0,
                Codegen.T1);
...
class DivideNode extends ArithmeticExpNode {
    ...
    public void codeGen() {
        ...
        // pop lhs value into %t0
        Codegen.genPop(Codegen.T0);
        // load quotient into $t0
        Codegen.generateWithComment("div", "quotient", Codegen.T0, Codegen.T0,
                Codegen.T1);
        // push quotient onto stack
        Codegen.genPush(Codegen.T0);
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        // push lhs result on the stack
        this.myExp1.codeGen();
        // pop lhs value into $t0
        Codegen.genPop(Codegen.T0);

        // generate label for rhs
        String falseLabel = ".F" + Codegen.nextLabel().substring(2);
        String doneLabel = ".D" + Codegen.nextLabel().substring(2);

        // jump to false if lhs is false
        Codegen.generate("beq", Codegen.T0, String.valueOf(0), falseLabel);

        // fall through if left hand side to true
        // to the rhs
        this.myExp2.codeGen();
        // pop rhs value into %t0
        Codegen.genPop(Codegen.T0);
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        // generate label for rhs
        String trueLabel = ".T" + Codegen.nextLabel().substring(2);
        String doneLabel = ".D" + Codegen.nextLabel().substring(2);

        // jump to true if lhs is true
        Codegen.generate("beq", Codegen.T0, String.valueOf(1), trueLabel);

        // fall through if left hand side to false
        // to the rhs
        this.myExp2.codeGen();
        // pop rhs value into %t0
        Codegen.genPop(Codegen.T0);

        // jump to true label if true
        Codegen.generate("beq", Codegen.T0, String.valueOf(1), trueLabel);
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        ...
        // push the left variable onto the stack
        this.myExp1.codeGen();
        // push the right variable onto the stack
        this.myExp2.codeGen();

        // pop right variable into $t1
        Codegen.genPop(Codegen.T1);
        // pop right variable into $t0
        Codegen.genPop(Codegen.T0);

        // load equality result into $t0
        Codegen.generateWithComment("seq", "compare for equality", Codegen.T0,
                Codegen.T0, Codegen.T1);

        // push equality result onto stack
        Codegen.genPush(Codegen.T0);
...
class NotEqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        ...
        // load non-equality result into $t0
        Codegen.generateWithComment("sne", "compare for non-equality",
                Codegen.T0, Codegen.T0, Codegen.T1);

        // push non-equality result onto stack
        Codegen.genPush(Codegen.T0);
...
class LessNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // put result in $T0
        Codegen.generateWithComment("slt", "less than", Codegen.T0, Codegen.T0,
                Codegen.T1);
...
class GreaterNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // put result in $T0
        Codegen.generateWithComment("sgt", "greater than", Codegen.T0,
                Codegen.T0, Codegen.T1);
        // push result onto stack
        Codegen.genPush(Codegen.T0);
...
class LessEqNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // put result in $T0
        Codegen.generateWithComment("sle", "less than or equal to", Codegen.T0,
                Codegen.T0, Codegen.T1);
        // push result onto stack
        Codegen.genPush(Codegen.T0);
...
class GreaterEqNode extends RelationalExpNode {
    ...
    public void codeGen() {
        ...
        // put result in $T0
        Codegen.generateWithComment("sge", "greater than or equal to",
                Codegen.T0, Codegen.T0, Codegen.T1);
        // push result onto stack
        Codegen.genPush(Codegen.T0);
