...
public class Driver {
	...
	/**
	 * Driver constructor for client programs and testers. Note that
	...
	/**
	 * If we are directly invoking Driver from the command line, this
	...
	public int process() {
		...
		if (ErrMsg.getErr()) {
			return Driver.RESULT_SYNTAX_ERROR;
		}

		astRoot.nameAnalysis();	 // perform name analysis
		if (ErrMsg.getErr()) {
			return Driver.RESULT_NAME_ANALYSIS_ERROR;
		}

		astRoot.typeCheck();
		if (ErrMsg.getErr()) {
			return Driver.RESULT_TYPE_ERROR;
		}
	...
	public static void main(String[] args) {
		Driver instance = new Driver(args);
		instance.run();
	}
}
